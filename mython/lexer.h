...
public:
    ...
    const T& ExpectNext() {
        ...
        if (next_token.Is<T>()) {
            return next_token.As<T>();
        } else {
            throw LexerError("Next_token_has_bad_type");
        }
    }
    ...
    void ExpectNext(const U& value) {
        ...
        if (next_token.Is<T>()) {
            if (next_token.As<T>().value == value) {
                ...
                throw LexerError("Next_token_has_bad_value");
            }
        } else {
            throw LexerError("Next_token_has_bad_type");
        }
    ...
    bool ParseKeyLexem(const std::string& lexem);

    void ParseIdLexem(const std::string& lexem);

    void ParseDigitLexem(const std::string& lexem);

    void ParseCharLexem(char lexem, std::istream& input);

private:
    std::vector<Token> tokens_;
    int current_token_ = 0;
    int max_token_;
    int current_expect_ = 0;
};
